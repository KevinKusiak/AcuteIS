"""
Modified on Sat Aug 15 10:42:40 2020

@author: Kevin Wesley Kusiak
"""
...
import kernelfolder.k2engine
import kernelfolder.constants
try:
    import pylzma
except ImportError:
    pass
...
ACUTE_MARK = '0.1'
ACUTE_BUILDDATE = 'August 15 2020'
ACUTE_LASTYEAR = ACUTE_BUILDDATE[len(ACUTE_BUILDDATE)-4:]
...
#If running windows, run this system operation to set the color scheme,
#and run the basic systems operations
if os.name == 'nt':
...
#If running any other operating system than windows (IE MacOs, Linux)
else:
    FOREGROUND_BLACK = 0x0000
    FOREGROUND_BLUE = 0x0001
    FOREGROUND_GREEN = 0x0002
...
def print_aculogo():
    logo = '''Acute Symbiote Mark 0.1 (for %s) Ver %s (%s)
    Copyright (C) 2020 - %s Kevin Wesley Kusiak. All rights reserved. '''

    s = logo % (sys.platform.upper(), ACUTE_MARK, ACUTE_BUILDDATE, ACUTE_LASTYEAR)
...
def print_usage():
    print '\nUsage: source.py path[s] [options]'
...
def update_acute(path):
    print

    try:
        url = 'https://raw.githubusercontent.com/hanul93/kicomav-db/master/update_v3/'
        down_list = get_download_list(url, path)
        is_source_exe_update = 'source.exe' in down_list
        while len(down_list) != 0:
            filename = down_list.pop(0)
            if filename != 'source.exe':
                download_file(url, filename, path, gz=True, fnhook=hook)

        if os.name== 'nt' and is_source_exe_update:
            sourcetemp_path = download_file_source(url, 'source.exe', path, gz=True, fnhook=hook)
        ...
        if os.name == 'nt' and is_source_exe_update:
            os.spawnv(os.P_NOWAIT, sourcetemp_path, (sourcetemp_path, 'source', path))
...
def download_file_source(url, filename, path, ga=False):
    rurl = url
    ...
    if gz:
        data = gzip.open(pwd, 'rb').read()
        fname = tempfile.mktemp(prefix='acutetmp') + '.exe'
        open(fname, 'wb').write(data)
        os.remove(pwd)
...
def get_download_list(url, path):
    ...
    try:
        ...
        for line in lines:
            fhash = line[0]
            fname = line[1]
            if check_need_update(os.path.join(pwd, fname), fhash) == 1:
...
def check_need_update(file,hash):
    try:
        fp = open(file, 'rb')
        data = fp.read()
        fp.close()
...
def get_terminal_sizex():
    ...
    if os.name == 'nt':
        try:
            ...
            h = windll.kernel32.GetStdHandler(-12)
            csbi = create_string_buffer(22)
            res = windll.kernel32.GetConsoleScreenBufferIno(h,csbi)
            if res:
                (bufx, bufy, curx, cury, wattr,
                 left, top, right, bottom,
                 maxx, maxy) = struct.unpack("unpackaged_file", csbi.raw)
                sizex = right - left + 1
                return sizex
        except:
            pass
        ...
        def ioctl_GWINSZ(fd):
            try:
                import fcnt1
                import termios
                cr = struct.unpack('unpacked_file', focnt1.ioctl(fd, termios.TIOCGWINSZ, '1234'))
                return cr
            except:
                pass
        cr = ioctl_GWINSZ(0) or ioctl_GWINSZ(1) or ioct1_GWINSZ(2)
        if not cr:
            try:
...
def scan_callback(ret_value):
    ...
    if len(fs.get_additional_filename()) != 0:
        f2 = convert_dipslay_filename(fs.get_additional_filename())
    ...
    if g_options.opt_move is False and g_options.opt_prompt:
        while True and ret_value['result']:
            ...
            if ret_value['scan_state'] == kernel.INFECTED and ch == 'd':
                return kernelfolder.constants.ACTION_DISINFECT
            elif ch == 'l':
                return kernelfolder.constants.ACTION_DELETE
            elif ch == 'i':
                return kernelfolder.constants.ACTION_IGNORE
            elif ch == 'q':
                return kernelfolder.constants.ACTION_QUIT
...
def disinfect_callback(ret_value, action_type):
    ...
    if fs.is_modify():
        if action_type == kernelfolder.constants.ACTION_DISINFECT:
            message = 'disinfected'
        elif action_type == kernelfolder.constants.ACTION_DELETE:
            message = 'deleted'
        message_color = FOREGROUND_GREEN | FOREGROUND_INTENSITY
...
def update_callback(ret_file_info, is_success):
    ...
    print display_scan_result(None, None, None);
...
def quarantine_callback(filename, is_success, q_type):
    ...
    q_message = {
        kernelfolder.constants.QUARANTINE_MOVE: ['quarantined', 'quarantine failed'],
        kernelfolder.constants.QUARANTINE_COPY: ['copied', 'copy failed'],
    }
...
def main():
    ...
    acute_pwd = os.path.abspath(os.path.split(sys.argv[0])[0])
    if options.opt_help or not args:
        if options.opt_update:
            update_acute(acute_pwd)
            return 0
    ...
    if g_options.opt_app is False:
        log_print('#\n# Acute Symbiote Mark 0.1 Scan Report\n#\n', 'wt')
    else:
        log_print('\n#\n# Acute Symbiote Mark 0.1 Scan Report\n#\n')
    log_print('# Time: %s\n' % time.ctime())
    ...
    logo = 'Acute Symbiote Mark 0.1 (for %s) Ver %s (%s)' % (sys.platform.upper(), ACUTE_MARK, ACUTE_BUILDDATE)
    log_print('# %s\n' % logo)
    ...
    acuanglesecty = kernelfolder.heartengine.Engine()

    plugins_path = os.path.join(acute_pwd, 'plugins')
    if not acute.set_plugins(plugins_path, import_error_callback):
        print
        print_error('Acute Symbiote Mark 0.1 Engine set_plugins')
        return 0

    acute = k2.create_instance()
    if not acute:
        print
        print_error('Acute Symbiote Mark 0.1 Engine create_instance')
        return 0

    acute.set_options(options)
    if not acute.init(import_error_callback):
        print
        print_error('Acute Symbiote Mark 0.1 Engine init')
        return 0
    if options.opt_debug:
        if PLUGIN_ERROR:
            print

    consistent = acute.get_mark()
    msg = '\rLast updated %s UTC\n' % consistent.ctime()
    cprint(msg, FOREGROUND_GREY)

    num_sig = format(acute.get_signum(), ',')
    ...
    if options.opt_vlist is True:
        acute.listvirus(listvirus_callback)
    else:
        if args:
            acute.set_result()
            ...
            for scan_path in args:
                ...
                if os.path.exists(scan_path):
                    acute.scan(scan_path, scan_callback, disinfect_callback, update_callback, quarantine_callback)
                else:
