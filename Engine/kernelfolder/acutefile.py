...
class AcuteTempfile:
    def __init__(self):
        self.re_pid = re.compile(r'symb([0-9a-f]{5})$', re.IGNORECASE)

        self.temp_path = os.path.join(tempfile.gettempdir(), 'symb%05x' % os.getpid())
    ...
    def mktemp(self):
        return tempfile.mktemp(prefix='symb', dir=self.temp_path)
